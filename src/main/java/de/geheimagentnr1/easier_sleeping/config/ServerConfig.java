...
public class ServerConfig {
	...
	static {
		...
		DIMENSIONS = BUILDER.comment( String.format(
			...
			"dimensions",
			Collections.singletonList( Objects.requireNonNull( World.field_234918_g_.func_240901_a_() ).toString() ),
			o -> {
	...
	public static synchronized void invertDimensions() {
		...
		for( ServerWorld serverworld : ServerLifecycleHooks.getCurrentServer().getWorlds() ) {
			...
			if( !dimensions.contains( registrykey ) ) {
				newDimensionRegistryNames.add( Objects.requireNonNull( registrykey.func_240901_a_() ).toString() );
			}
		}
